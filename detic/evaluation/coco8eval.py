...
class COCO8Eval:
    def __init__(
        ...
        """Constructor for COCO-8 80 evaluation.
...
def _evaluate_predictions_on_coco8(
        coco8_gt,                # ground-truth annotation file
        coco8_results_path,      # model prediction results saved from inference stage
        eval_seg=False,
    ...
    results = {}
    coco8_eval = COCO8Eval(coco8_gt, coco8_results_path, 'bbox', expand_pred_label=False)
    coco8_eval.run()
    coco8_eval.print_results()
    results["AP50"] = coco8_eval.get_results()["AP50"]

    mAP = np.zeros(len(class_names)) - 1

    precisions = coco8_eval.eval['precision']
    assert len(class_names) == precisions.shape[2]

    results_per_category = []
    id2apiid = sorted(coco8_gt.get_cat_ids())
    ...
    for idx, name in enumerate(class_names):
        ...
        inst_num = len(coco8_gt.get_ann_ids(cat_ids=[id2apiid[idx]]))
        if inst_num > 0:
    ...
    inst_aware_ap = inst_aware_ap * 100 / inst_count if inst_count > 0 else 0
...
class COCO8Evaluator(DatasetEvaluator):
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        self._coco8_api = LVIS(json_file)
        # Test set json files do not contain annotations (evaluation must be
        # performed using the LVIS evaluation server).
        self._do_evaluation = len(self._coco8_api.get_ann_ids()) > 0
    ...
    def evaluate(self):
        ...
        self._logger.info("Preparing results in the coco8 500 format ...")
        self._coco8_results = list(
            itertools.chain(*[x["instances"] for x in self._predictions]))

        # unmap the category ids for LVIS (from 0-indexed to 1-indexed)
        for result in self._coco8_results:
            result["category_id"] += 1

        PathManager.mkdirs(self._output_dir)
        file_path = os.path.join(
            self._output_dir, "coco8_instances_results.json")
        self._logger.info("Saving results to {}".format(file_path))
        with PathManager.open(file_path, "w") as f:
            f.write(json.dumps(self._coco8_results))
            f.flush()
        ...
        res, mAP = _evaluate_predictions_on_coco8(
            self._coco8_api,
            file_path,
        ...
        mAP_out_path = os.path.join(self._output_dir, "coco8_mAP.npy")
