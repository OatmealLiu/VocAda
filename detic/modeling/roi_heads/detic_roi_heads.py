...
# This class is the classification module used by Swin-B series
...
class DeticCascadeROIHeads(CascadeROIHeads):
    ...
    def _forward_box(
        ...
        # Miu: this loop calls box_predictor
        ...
        if self.training:
            ...
            # >>>>>> Miu >>>>>>
            # Miu: here 6
            # Miu: mask out non-CoI-proposed classes' scores!
            # Creating a mask with all False values
            mask = torch.zeros(scores[0][0].shape, dtype=torch.bool)
            if spotlight_info is not None:
                print(spotlight_info)     # echo proposal IDs to terminal for debug purposes
                for coi_id in spotlight_info:
                    mask[coi_id - 1] = True   # Miu: coi_id is 1-indexed, so that we convert it to 0-indexed
            else:
                for i in range(len(mask)):
                    mask[i] = True

            mask[-1] = True  # Miu: for the <no object> vector
            # setting all other elements to 0
    ...
    # Miu: here 3
    def forward(
            self, images, features, proposals, targets=None, ann_type='box', classifier_info=(None, None, None),
            spotlight_info=None,
    ):
        """
        ...
        if self.training:
            ...
            # pred_classes decided
            # Miu: here 4
            pred_instances = self._forward_box(
                features, proposals, classifier_info=classifier_info, spotlight_info=spotlight_info
            )
            # a = pred_instances[0]._fields['pred_classes']


            pred_instances = self.forward_with_given_boxes(features, pred_instances)

            # pred_classes filed is the same
            return pred_instances, {}
