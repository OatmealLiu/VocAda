...
class CustomRCNN(GeneralizedRCNN):
    ...
    def inference(
        ...
        proposals, _ = self.proposal_generator(images, features, None)      # CerternetV2
        ...
        results, _ = self.roi_heads(
            ...
            features,
            proposals,
            spotlight_info=batched_inputs[0].get("spotlight_cois", None),
        )
    ...
    def forward(
        ...
        """
        This is the function used in forward inference in inference_on_dataset function
        Add ann_type
        Ignore proposal loss when training with image labels
        """

        # if eval==True, run inference directly
        if not self.training:
            return self.inference(batched_inputs)           # Miu: here 1
    ...
    def _sync_caption_features(
            self, caption_features, ann_type, BS
    ):
    ...
    def _sample_cls_inds(
            self, gt_instances, ann_type='box'
    ):
        if ann_type == 'box':
