...
def spotlight_cls_test(
        model, proposed_coi_embedding, num_classes
):
    """
    Given the running model and the encoded proposed CoIs
    this function will reset the classifier weights
    """
    model.roi_heads.num_classes = num_classes

    if proposed_coi_embedding:
        # Swap C x D to D x C
        zs_weight = proposed_coi_embedding.permute(1, 0).contiguous()
        # D x (C + 1), add one more zero-like dummy class
        zs_weight = torch.cat(
            [zs_weight,
             zs_weight.new_zeros((zs_weight.shape[0], 1))],
            dim=1,
        )
    else:
        zs_weight = torch.zeros((512, 1), dtype=torch.float32)

    # Normalize and handle both architecture
    try:
        # Attempt to access the first structure
        ...
        try:
            ...
            # Handle the case where neither condition is met, if necessary
            raise AttributeError("Miu: SpotDet/detic/modeling/utils.py Line 61 ERROR!!!")
    ...
    try:
        for k in range(len(model.roi_heads.box_predictor)):
            ...
            # Overwrite, reset the classifier weight
