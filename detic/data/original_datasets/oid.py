...
for key, (image_root, json_file) in _PREDEFINED_SPLITS_OID.items():
    register_oid_instances(
        ...
        os.path.join("datasets", json_file) if "://" not in json_file else json_file,   # COCO-styled annotation file
        os.path.join("datasets", image_root),   # dataset root
    )
...
for key, (image_root, json_file) in _PREDEFINED_SPLITS_OID_HIERARCHY.items():
    register_oid_instances(
        key,    # dataset split name that matches the COMMAND LINE args
        _get_builtin_metadata(_PREDEFINED_HRCHY_CATEGORIES[key]),  # label info. {'id': [...], 'ClassNames': [...]}
        os.path.join("datasets", json_file) if "://" not in json_file else json_file,   # COCO-styled annotation file
        os.path.join("datasets", image_root),   # dataset root
    )
...
#         "oid/images/validation/", "oid/annotations/oid_challenge_2019_val_expanded_rare.json"),
#     }
#
#     for key, (image_root, json_file) in _PREDEFINED_SPLITS_OID.items():
#         register_oid_instances(
...
#         register_oid_instances(
#             key,  # dataset split name that matches the COMMAND LINE args
#             _get_builtin_metadata(_PREDEFINED_SPLITS_OID_HIERARCHY[key]),
