...
def format_class_prompt_with_synonyms_numbers(dict_class_synonyms):
    prompt = ""
...
def do_llm_number_proposal(args, wandb_run, debug_knob=False):
    def postprocess_proposal(raw_proposal, chunked_names):
        parent_pattern = r'\((\d+)\)'
        indices = [int(num) - 1 for num in re.findall(parent_pattern, raw_proposal)]

        proposals = []
        for i in indices:
            if 0 <= i < len(chunked_names):
                proposals.append(chunked_names[i])
        return proposals


    wandb_columns = ['image_id', 'file_name', 'captions', 'nouns', 'proposals']
    proposal_table = wandb.Table(columns=wandb_columns)
    ...
    if 'gpt' in args.llm_model_name:
        ...
        agent_llm = LLaMA3sAlpha(model=args.llm_model_name, temperature=0.6, max_new_tokens=256)
    ...
    # load llm prompt
    template_llm_sys_prompt = load_txt(args.llm_prompt_proposing)
    ...
    for this_img_entry in tqdm(captioned_results):
        ...
        for this_caption_entry in this_img_entry['answers_llava']:
            ...
            this_cap_sys_prompt = (template_llm_sys_prompt
                                   .replace("[__CAPTION__]",         prompt_caption_text)
                                   .replace("[__EXTRACTED_NOUNS__]", prompt_extracted_nouns))

            # record it to wandb
            wandb_captions += '*********\n' + prompt_caption_text    + '*********\n'
            wandb_nouns    += '*********\n' + prompt_extracted_nouns + '*********\n'
            # infer
            print(60 * "=")
            clrprint(this_cap_sys_prompt)
            ...
            for achunk_names in chunked_class_names:
                ...
                prompt_class_names_with_synonyms = format_class_prompt_with_synonyms_numbers(
                    {k: full_vocabulary_dict_with_synonyms[k]['synonyms'] for k in achunk_names}
                )


                clrprint('\n' + prompt_class_names_with_synonyms + '\n', c='light_green')

                raw_reply = agent_llm.infer(prompt_class_names_with_synonyms,
                                            system_prompt=this_cap_sys_prompt,
                                            )

                print("******** Raw Reply:\n")
                clrprint(raw_reply, c='light_blue')

                vanilla_reply = postprocess_proposal(raw_reply, achunk_names)
        ...
        proposal_table.add_data(
            num_img_index,
            this_img_entry['file_name'],
            wandb_captions,
            wandb_nouns,
            ", ".join(this_img_entry['vanilla_proposals'])
        )
    ...
    wandb_run.log({'table_key': proposal_table})
