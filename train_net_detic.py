...
from detic.evaluation.coco8eval import COCO8Evaluator

logger = logging.getLogger("detectron2")


def do_test(cfg, model, confThr=0.02):
    results = OrderedDict()
    for d, dataset_name in enumerate(cfg.DATASETS.TEST):
        evaluator_type = MetadataCatalog.get(dataset_name).evaluator_type

        if cfg.MODEL.RESET_CLS_TESTS:
            if (evaluator_type == 'coco') and ('objects365' not in dataset_name):
                reset_cls_test_coco(
                    model,
        ...
        # if cfg.MODEL.RESET_CLS_TESTS and evaluator_type == 'coco':
        #     reset_cls_test_coco(
        #         model,
        #         cfg.MODEL.TEST_CLASSIFIERS[d],
        #         cfg.MODEL.TEST_NUM_CLASSES[d]
        #     )
        # elif cfg.MODEL.RESET_CLS_TESTS:
        #     reset_cls_test(
        #         model,
        #         cfg.MODEL.TEST_CLASSIFIERS[d],
        #         cfg.MODEL.TEST_NUM_CLASSES[d]
        ...
        # where to output the detection .JSON
        output_folder = os.path.join(cfg.OUTPUT_DIR, "inference_{}_confThr={}".format(dataset_name, confThr))

        # create evaluator
        if "lvis" in evaluator_type or cfg.GEN_PSEDO_LABELS:
            evaluator = LVISEvaluator(dataset_name, cfg, True, output_folder)
        elif 'coco' in evaluator_type:
            if 'coco_generalized_zeroshot_val' in dataset_name:
        ...
        elif evaluator_type == 'tools':
            evaluator = FSODEvaluator(dataset_name, cfg, True, output_folder)
        elif evaluator_type == 'coco8':
            evaluator = COCO8Evaluator(dataset_name, cfg, True, output_folder)
        else:
        ...
        #     print(60*"+")
        #     print(
        #         model.roi_heads.box_predictor[k]
        #     )
        #     print(
        #         model.roi_heads.box_predictor[k].cls_score
        #     )
        #     print(
        #         model.roi_heads.box_predictor[k].cls_score.zs_weight.shape
        #     )
...
def setup(args):
    ...
    # Miu: Set score_threshold for builtin models
    cfg.MODEL.RETINANET.SCORE_THRESH_TEST = args.confThr
    cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = args.confThr
    cfg.MODEL.PANOPTIC_FPN.COMBINE.INSTANCES_CONFIDENCE_THRESH = args.confThr
...
def main(args):
    ...
    if args.eval_only:
        ...
        return do_test(cfg, model, confThr=args.confThr)
    # <<< eval only <<<
...
if __name__ == "__main__":
    ...
    # Miu: to test different confidence thresholds
    args.add_argument(
        "--confThr",
        type=float,
        default=0.02,
        help="Minimum score for valid instance predictions",
    )
